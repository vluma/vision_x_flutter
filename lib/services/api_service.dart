...
class ApiService {
  ...
  // 按API搜索
  static Future<List<MediaDetail>> _searchByAPI(String apiCode, String apiUrl, String apiName, String query) async {
    try {
      ...
      final searchUrl = '$apiUrl?ac=videolist&wd=${Uri.encodeQueryComponent(query)}';
  ...
  static MediaDetail _mapApiDataToMediaDetail(dynamic item, String apiName, String apiCode, String apiUrl) {
    // 解析剧集信息
    List<Source> sources = _parseEpisodes(item['vod_play_url']?.toString() ?? '');
    
    return MediaDetail(
      id: item['vod_id'] is int ? item['vod_id'] : int.tryParse(item['vod_id']?.toString() ?? '0') ?? 0,
      ...
      scoreAll: item['vod_score_all'] is int ? item['vod_score_all'] : int.tryParse(item['vod_score_all']?.toString() ?? '0') ?? 0,
      scoreNum: item['vod_score_num'] is int ? item['vod_score_num'] : int.tryParse(item['vod_score_num']?.toString() ?? '0') ?? 0,
      doubanScore: item['vod_douban_score']?.toString(),
      doubanId: item['vod_douban_id'] is int ? item['vod_douban_id'] : int.tryParse(item['vod_douban_id']?.toString() ?? '0') ?? 0,
      hits: item['vod_hits'] is int ? item['vod_hits'] : int.tryParse(item['vod_hits']?.toString() ?? '0') ?? 0,
      hitsDay: item['vod_hits_day'] is int ? item['vod_hits_day'] : int.tryParse(item['vod_hits_day']?.toString() ?? '0') ?? 0,
      hitsWeek: item['vod_hits_week'] is int ? item['vod_hits_week'] : int.tryParse(item['vod_hits_week']?.toString() ?? '0') ?? 0,
      hitsMonth: item['vod_hits_month'] is int ? item['vod_hits_month'] : int.tryParse(item['vod_hits_month']?.toString() ?? '0') ?? 0,
      up: item['vod_up'] is int ? item['vod_up'] : int.tryParse(item['vod_up']?.toString() ?? '0') ?? 0,
      down: item['vod_down'] is int ? item['vod_down'] : int.tryParse(item['vod_down']?.toString() ?? '0') ?? 0,
      time: item['vod_time']?.toString(),
      timeAdd: item['vod_time_add'] is int ? item['vod_time_add'] : int.tryParse(item['vod_time_add']?.toString() ?? '0') ?? 0,
      ...
      total: item['vod_total'] is int ? item['vod_total'] : int.tryParse(item['vod_total']?.toString() ?? '0') ?? 0,
      isEnd: item['vod_isend'] is int ? item['vod_isend'] : int.tryParse(item['vod_isend']?.toString() ?? '0') ?? 0,
      trysee: item['vod_trysee'] is int ? item['vod_trysee'] : int.tryParse(item['vod_trysee']?.toString() ?? '0') ?? 0,
      sourceName: apiName,
      sourceCode: apiCode,
      apiUrl: apiUrl,
