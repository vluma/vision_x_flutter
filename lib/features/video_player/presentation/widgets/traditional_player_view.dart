...
class TraditionalPlayerView extends ConsumerWidget {
          ...
          children: [
            ...
            if (uiState.controlsVisible || !playerState.isPlaying)
              NormalControlsView(
                ...
                onToggleFullScreen: () {
                  ref.read(videoPlayerConfigProvider(params.media).notifier).toggleFullScreen();
                },
                onNextEpisode: _canPlayNext(ref) ? () => _playNextEpisode(ref) : null,
                onPrevEpisode: _canPlayPrev(ref) ? () => _playPrevEpisode(ref) : null,
                onSpeedChanged: (speed) => playerNotifier.setPlaybackSpeed(speed),
              ),
  ...
  void _playNextEpisode(WidgetRef ref) {
    ...
    if (currentIndex < source.episodes.length - 1) {
      ...
      // 更新当前剧集
      ref.read(currentEpisodeProvider(params.media).notifier).state = nextEpisode;
      ref.read(currentEpisodeIndexProvider(params.media).notifier).state = nextIndex;
  ...
  void _playPrevEpisode(WidgetRef ref) {
    ...
    if (currentIndex > 0) {
      ...
      // 更新当前剧集
      ref.read(currentEpisodeProvider(params.media).notifier).state = prevEpisode;
      ref.read(currentEpisodeIndexProvider(params.media).notifier).state = prevIndex;
